...
/**
 * GPX is read by hand rather than through an XML parser, and that is the safer choice here, not
 * a shortcut: nothing below resolves an external entity or expands a nested one, so neither XXE
 * nor a billion-laughs document has anything to act on. The grammar being read is also tiny —
 * `wpt`, `rte`, `trk` and four text children — and fixed by the GPX 1.1 schema.
 */
...
function xmlChildText(inner: string, tag: string): string {
